...
struct Cloner : Module {
    ...
    void dataFromJson(json_t *rootJ) override {
        ...
        if(clippingJ) clipping =  json_integer_value(clippingJ);
...
void Cloner::process(const ProcessArgs &args) {
    ...
    if (outputs[CV_OUTPUT].isConnected()) {
        ...
        if(clipping) {
            ...
            for(int c=0; c<channels; c++) outputs[CV_OUTPUT].setVoltage(cv + (c-offset)*spread/(div), c);
        };
    }
}
...
struct ClonerClippingItem : MenuItem {

    Cloner *cloner;

    void onAction(const event::Action &e) override {
        cloner->clipping = !cloner->clipping;
    };

    void step() override {
        rightText = (cloner->clipping)? "✔" : "";
    };
...
void ClonerWidget::appendContextMenu(Menu *menu) {
    ...
    ClonerClippingItem *clipping_Item = new ClonerClippingItem();
    clipping_Item->text = "Clipping";
    clipping_Item->cloner = cloner;
    menu->addChild(clipping_Item);
